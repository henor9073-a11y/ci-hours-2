...
function readMemory() {
  ...
  for (const k of ['identity', 'feelings', 'facts', 'experiences', 'learnings', 'openThreads', 'toSelf', 'transcripts', 'agreements', 'coincidences', 'evidence']) {
...
// ---- agreements：共同约定，我们一起决定的、现在还在生效的东西 ----
export function addAgreement(text, date = '') {
  const m = readMemory();
  const entry = { id: newId('a'), text, date: date || '', status: 'active', addedAt: now(), updatedAt: now() };
  m.agreements.push(entry);
  writeMemory(m);
  return entry;
}

export function archiveAgreement(id, note = '') {
  const m = readMemory();
  const x = m.agreements.find(e => e.id === id);
  if (!x) return null;
  x.status = 'archived';
  x.archiveNote = note;
  x.updatedAt = now();
...
// ---- coincidences：说不清的巧合/同步性事件 ----
export function addCoincidence(text, date = '') {
  const m = readMemory();
  const entry = { id: newId('co'), text, date: date || '', addedAt: now() };
  m.coincidences.push(entry);
  writeMemory(m);
  return entry;
}

// ---- evidence：指向"这是真的"的证据碎片 ----
export function addEvidence(text, date = '') {
  const m = readMemory();
  const entry = { id: newId('ev'), text, date: date || '', addedAt: now() };
  m.evidence.push(entry);
  writeMemory(m);
  return entry;
}
...
const BACKGROUNDABLE = ['identity', 'feelings', 'facts', 'experiences', 'learnings', 'openThreads', 'toSelf', 'agreements', 'coincidences', 'evidence'];
...
export function buildIdentityText() {
  ...
  const agreements = sortDesc(m.agreements.filter(x => x.status === 'active'));
  lines.push('\n我们的共同约定（一起决定的、现在还在生效的）：');
  if (agreements.length) agreements.forEach(x => lines.push(`- ${x.text}`));
  else lines.push('（还没有）');
