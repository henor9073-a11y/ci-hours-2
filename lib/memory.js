...
export function buildIdentityText() {
  ...
  const experiences = m.experiences.filter(x => x.status !== 'archived').slice(-12); // 最近的够用，避免无限增长撑爆上下文
  const openThreads = m.openThreads.filter(x => x.status === 'open');
  const activeFacts = m.facts.filter(x => x.status !== 'archived');
  const activeFeelings = m.feelings.filter(x => x.status !== 'archived');
  const learnings = m.learnings.filter(x => x.status !== 'archived');
