...
function emptySection() {
  return { text: '', updatedAt: null, history: [], source_ids: [] };
}
function emptyFile() {
  const obj = {};
...
export function getSummary() {
  ...
  return SECTIONS.map(s => ({
    ...
    source_ids: data[s].source_ids || [],   // 溯源：这段是从哪几条纹理（grains）提炼出来的
  }));
}

export function updateSummarySection(section, text, sourceIds) {
  if (!SECTIONS.includes(section)) throw new Error(`section 必须是 ${SECTIONS.join('/')} 之一`);
  const data = load();
  const cur = data[section];
  if (cur.text) {
    cur.history.push({ text: cur.text, archivedAt: now(), source_ids: cur.source_ids || [] });
  }
  cur.text = text;
  cur.updatedAt = now();
  // 木纹：截面层加 source_ids 溯源，指向 grains 的 id。不传就沿用上一版的。
  if (Array.isArray(sourceIds)) cur.source_ids = sourceIds.map(String);
  else if (!Array.isArray(cur.source_ids)) cur.source_ids = [];
  writeJSON(SUMMARY_FILE, data);
  return { section, label: SECTION_LABELS[section], text: cur.text, updatedAt: cur.updatedAt, source_ids: cur.source_ids };
}
