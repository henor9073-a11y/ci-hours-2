...
export function enqueueSpeech(text, voiceId = null) {
  const q = readJSON(SPEECH_FILE, []);
  for (const item of q) if (item.status === 'pending') item.status = 'skipped';
  // voiceId 指向 voice.js 那边已经生成好的音频文件（/api/voice/:id/audio）。
  // 如果生成失败，voiceId 是 null——网页会收到这条但没法真的播出声音，只是排进去而已。
  const entry = { id: newId(), text, voiceId, status: 'pending', createdAt: new Date().toISOString() };
  q.push(entry);
