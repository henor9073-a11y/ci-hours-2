...
import { clipToSentence, bigrams, nearDuplicate } from './search.js';
...
export function formatInjection(result) {
  ...
  for (const x of people) {
    if (x.person) lines.push(`· [人] ${formatPerson(x.person)}`);
    else lines.push(`· [人] ${x.matched}（还没有卡片，只有八卦）`);
    for (const g of x.gossip) lines.push(`  · [八卦 ${g.date}] ${clipToSentence(g.text.replace(/\s+/g, ' '), 160)}`);
  }
  for (const m of result.memories || []) {
    if (m.layer === 'semantic') {
      const conf = m.confidence === 'cite' ? '' : '（记得不太牢，留点余地）';
      lines.push(`· [语义·${m.category}·heat${m.heat}] ${m.date ? m.date + ' ' : ''}${clipToSentence(m.text.replace(/\s+/g, ' '), 200)}${conf}${m.reason ? `　← ${m.reason}` : ''}`);
      ...
      // 纹理正文跟年轮片段一个道理：硬切 200 字会断在半句话上
      // （"棋子说\"0/10。我不（记得不太牢…"），截到句号读起来才是一句话。
      lines.push(`· [${m.category}·heat${m.heat}] ${m.date ? m.date + ' ' : ''}${clipToSentence(m.text.replace(/\s+/g, ' '), 200)}${conf}`);
    }
  }
  return lines.join('\n');
}
