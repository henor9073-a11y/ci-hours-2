...
export async function autoRecall(query, { maxReturn = 3, minScore = 6, useAgent = null, context = '', touchHits = false, excludeIds = [], queries = null } = {}) {
  ...
  const picked = grainHits.slice(0, maxReturn);

  // 第二层：纹理一条都没给出来，翻年轮找线索（last_resort，只当参考不当权威）
  let rawHits = [];
  if (!picked.length) {
    for (const a of angles) {
      rawHits.push(...searchRings({ query: a, limit: 3 }).filter(r => r.score >= minScore));
      if (rawHits.length >= maxReturn) break;
    }
    rawHits = [...new Map(rawHits.map(r => [r.id, r])).values()].sort((a, b) => b.score - a.score);
  ...
  const memories = [
    ...picked.map(g => ({
      ...
      score: g.score, matched_angles: g._angles
    })),
