...
export async function autoRecall(query, { maxReturn = 3, minScore = 6, useAgent = null, context = '', touchHits = false, excludeIds = [], queries = null, useSemantic = null, _semanticPick = null } = {}) {
  ...
  // 第三层：纹理和语义都空，翻年轮找线索（last_resort，只当参考不当权威）
  let rawHits = [];
  if (!picked.length && !semanticMemories.length) {
    for (const a of angles) {
