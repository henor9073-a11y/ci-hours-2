...
export function updateCountdown(id, patch = {}) {
  const list = load();
  const c = list.find(x => x.id === id);
  if (!c) return null;
  if (patch.title !== undefined) { if (!String(patch.title).trim()) throw new Error('title 不能为空'); c.title = String(patch.title).trim(); }
  if (patch.note !== undefined) c.note = String(patch.note || '');
  if (patch.photo_id !== undefined) c.photo_id = String(patch.photo_id || '');
  if (patch.date !== undefined) {
    const d = String(patch.date);
    if (!/^\d{2}-\d{2}$/.test(d) && !/^\d{4}-\d{2}-\d{2}$/.test(d)) throw new Error('date 要是 MM-DD 或 YYYY-MM-DD');
    c.date = d; c.recurring = /^\d{2}-\d{2}$/.test(d);
  }
  c.updated_at = now();
  save(list);
  return c;
...
export function addCountdown(title, date, recurring, extra = {}) {
  ...
  const list = load();
  const entry = { id: newId('cd'), title: String(title).trim(), date, recurring: rec, note: String(extra.note || ''), photo_id: String(extra.photo_id || ''), created_at: now() };
  list.push(entry);
  save(list);
  return entry;
}
...
export function getCountdowns(limit) {
  ...
  const out = load().map(c => {
    c.note = c.note || ''; c.photo_id = c.photo_id || '';
    let next;
    if (c.recurring) {
      next = `${year}-${c.date}`;
