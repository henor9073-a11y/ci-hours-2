...
export * as moments from './moments.js';
export * as prefs from './prefs.js';
export * as quote from './quote.js';
export * as chat from './chat.js';
export * as emotions from './emotions.js';
export * as songs from './songs.js';
export * as firsts from './firsts.js';

import { searchGrains, CATEGORY_LABELS } from './grains.js';
...
export function searchAll({ query, layers = LAYERS, limit = 10 } = {}) {
  ...
  if (layers.includes('grains')) {
    for (const g of searchGrains({ query, limit })) {
      out.push({ layer: 'grains', id: g.id, score: g.score, category: g.category, label: CATEGORY_LABELS[g.category] || g.category, text: g.text, date: g.date || undefined, families: g.families && g.families.length ? g.families : undefined, heat: Math.round(g.heat), confidence: g.confidence });
    }
  }
  if (layers.includes('rings')) {
    for (const r of searchRings({ query, limit })) {
      out.push({ layer: 'rings', id: r.id, score: r.score, category: r.source_type, label: r.source_type === 'daily_summary' ? '每日总结' : '原始记录', window_name: r.window_name, title: r.title, date: r.date, source_type: r.source_type, excerpt: r.excerpt, length: r.length });
    }
  }
  if (layers.includes('profiles')) {
    for (const owner of OWNERS) {
      for (const p of getProfile(owner)) {
        ...
        if (s > 0) out.push({ layer: 'profiles', category: owner === 'cy' ? 'identity' : 'fact', label: owner === 'cy' ? '身份' : '事实', owner, field: p.field, score: s, excerpt: excerpt(p.content, query) });
      }
    }
  }
  if (layers.includes('photos')) {
    for (const p of searchPhotos(query, limit)) {
      out.push({ layer: 'photos', category: 'photo', label: '照片', id: p.id, score: p.score, date: p.date, caption: p.caption, tags: p.tags.length ? p.tags : undefined });
    }
  }
  if (layers.includes('cross_sections')) {
    for (const s of getSummary()) {
      const sc = score(query, s.text);
      if (sc > 0) out.push({ layer: 'cross_sections', category: 'summary', label: '摘要·' + s.label, section: s.section, score: sc, excerpt: excerpt(s.text, query) });
    }
  }
