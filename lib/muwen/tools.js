...
export const TOOLS = [
  ...
  {
    name: 'chat_reply',
    description: '回一条留言。content 是要说的话；thinking 可选（会折叠显示，棋子点开才看得到）；tools 可选（这次回复里调了哪些工具，会显示成一行小卡片）。回完记得用 chat_mark_read 把看过的标掉。',
    inputSchema: obj({
      content: str('要说的话'),
  ...
  },
  {
    ...
    description: '现在就写一条今日一句：读昨天的每日总结，让模型写一句话，存成 add_note(kind=write)。服务器每天墨尔本 9:00 会自动跑一次（在 8:00 的自动总结之后），这个工具是手动补跑用的。今天已经写过就跳过，force=true 强制重写。',
...
export async function callMuwenTool(name, args = {}) {
  switch (name) {
    ...
    case 'chat_reply': need(args, 'content'); return chat.sendMessage({ sender: 'cy', type: 'text', content: args.content, thinking: args.thinking || '', tools: args.tools || [], reply_to: args.reply_to || '' });
    ...
    case 'write_daily_quote': return quote.writeDailyQuote({ force: !!args.force });
