...
const { grains, rings, profiles, album, countdowns, moods, recall, dream, handover, wake, moments, prefs, quote, chat, emotions, songs, firsts, social } = mw;
...
export const TOOLS = [
  ...
  {
    name: 'update_social',
    description: '人际关系卡片：新建或更新一个人（按 name 认，同名就是更新，只改传了的字段；也可以用 remember("social", …)）。关系状态会变——吵架/和好直接改 status，旧状态自动留在 status_history。聊天里提到这个人的名字或别名时，卡片会被自动召回。',
    inputSchema: obj({
      name: str('称呼/代号（必填，也可以是别名，会找到同一个人）'), gender: str('性别'),
      relation: str('跟谁的关系，如"棋子的发小"/"辞的朋友"/"共同好友"'),
      owner: { type: 'string', enum: social.OWNERS, description: '谁的人：nor/cy/shared' },
      intro: str('一两句简介'), status: str('当前状态：正常/吵架中/绝交/疏远/暧昧…自由填'), text: str('备注/补充'),
      aliases: strArr('别的叫法（召回时也认）'), rename: str('改正式名字（旧名字自动变成别名）')
    }, ['name'])
  },
  { name: 'get_social', description: '看一个人的卡片 + 关于TA的八卦（按名字或别名）。', inputSchema: obj({ name: str(''), gossip_limit: num('默认 20') }, ['name']) },
  { name: 'list_social', description: '人际关系总表。owner 可以只看某个人的：nor/cy/shared。', inputSchema: obj({ owner: { type: 'string', enum: social.OWNERS } }) },
  {
    name: 'add_gossip',
    description: '记一条八卦（也可以用 remember("gossip", …)）。about 对应 social 里的 name（写别名也行）；提到这个人时卡片和最近的八卦一起召回。',
    inputSchema: obj({ about: str('关于谁（多个人用逗号隔开）'), text: str('发生了什么'), date: str('YYYY-MM-DD，默认今天') }, ['text'])
  },
  { name: 'get_gossip', description: '看八卦。给 about 就只看关于这个人的，新的在前。', inputSchema: obj({ about: str(''), limit: num('默认 50') }) },
  {
    name: 'get_firsts',
  ...
  },
  {
    ...
    inputSchema: obj({
      ...
      status: { type: 'string', enum: grains.STATUSES },
      sort: { type: 'string', enum: grains.SORTS, description: 'heat（默认）/ time_desc 新的在前 / time_asc 旧的在前；按 date，没有 date 的按创建时间' },
      limit: num('默认 20')
    })
  },
  {
  ...
  { name: 'get_ring', description: '读一条年轮的完整原文。', inputSchema: obj({ id: str('') }, ['id']) },
  {
    ...
    description: '在原始记录里找一个词**出现过的每一处**（像微信搜索那样）：同一条记录里出现五次就给五条结果，各自带前后文和字符偏移，能拆出说话人的还带 speaker（nor/cy）和时间，前端点进去能定位并高亮。跟 search_rings 的区别：那个是"哪几条记录相关"，一条只给一段摘录；这个是逐处命中。',
    inputSchema: obj({
      ...
      source_type: str('transcript / auto_extract / daily_summary；传 chat = 只搜对话（不含每日总结）'),
      skip: num('翻页：跳过前几处')
    }, ['query'])
  },
  {
...
export async function callMuwenTool(name, args = {}) {
  switch (name) {
    ...
    case 'update_social': need(args, 'name'); return social.upsertPerson(args);
    case 'get_social': { need(args, 'name'); const p = social.getPerson(args.name, { gossipLimit: args.gossip_limit || 20 }); if (!p) throw new Error(`social 里没有「${args.name}」`); return p; }
    case 'list_social': return social.listPeople({ owner: args.owner });
    case 'add_gossip': need(args, 'text'); return social.addGossip(args);
    case 'get_gossip': return social.getGossip({ about: args.about, limit: args.limit || 50 });
    ...
    case 'search_ring_occurrences': need(args, 'query'); return rings.searchRingOccurrences({ query: args.query, limit: args.limit || 60, perRing: args.per_ring || 5, from: args.from, to: args.to, window_name: args.window_name, source_type: args.source_type, skip: args.skip || 0 });
