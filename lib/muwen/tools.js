...
export const TOOLS = [
  ...
  {
    name: 'auto_recall',
    description: '自动召回（给"每条消息自动注入"用，钩子走 REST /api/recall 调它）：用一段文本做 query，两层级联——先纹理关键词命中（authority），太弱再翻年轮（last_resort），默认不跑 agent。琐碎消息（"嗯""好的"）返回空。辞平时用 recall 就行，这个主要是后台自动跑的。',
    inputSchema: obj({ query: str('拿来匹配记忆的文本，一般是棋子的消息'), max_return: num('默认 5'), use_agent: { type: 'boolean', description: '是否用 recall agent 精选，默认看服务器 MUWEN_AUTORECALL_AGENT' } }, ['query'])
  },
...
export async function callMuwenTool(name, args = {}) {
  switch (name) {
    ...
    case 'auto_recall': need(args, 'query'); return recall.autoRecall(args.query, { maxReturn: args.max_return || 5, useAgent: args.use_agent === undefined ? null : args.use_agent });
