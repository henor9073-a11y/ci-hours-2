...
const { grains, rings, profiles, album, countdowns, moods, recall, dream } = mw;
...
export const TOOLS = [
  ...
  {
    name: 'get_calendar',
    description: '读最近 N 天的每日总结（日历式），按日期倒序。每条：{date, headline, nor_status, cy_status, pending, intimate}。intimate 直接在里面，不用额外参数。醒来第二步读这个，替代以前翻 daily_summary 的方式。',
  ...
  {
    name: 'add_daily',
    description: '写今天（或某天）的每日总结，存进年轮 source_type=daily_summary。同一天再写一次就整体覆盖（旧版留在历史里）。headline 一句话概括今天最重要的事；nor_status 棋子的状态；cy_status 我的状态；pending 未完成的事；intimate 当天的亲密行为记录（次数等），会直接显示在 get_calendar 里。',
    inputSchema: obj({
      date: str('YYYY-MM-DD'), headline: str(''), nor_status: str(''), cy_status: str(''), pending: str(''), intimate: str('')
    }, ['date', 'headline'])
  },
