...
export const TOOLS = [
  ...
  {
    name: 'save_photo',
    description: '存一张图片进相册（base64，单张 2MB 以内，jpeg/png/webp/gif）。',
    inputSchema: obj({
      image_base64: str('图片的 base64（可以带 data:...;base64, 前缀）'),
      mime_type: { type: 'string', enum: ['image/jpeg', 'image/png', 'image/webp', 'image/gif'] },
      description: str('简短描述'), date: str('YYYY-MM-DD，默认今天'), tags: strArr('可选标签')
    }, ['image_base64', 'mime_type'])
  },
  { name: 'list_photos', description: '按日期范围/标签浏览相册，只返回元数据不返回图片数据。', inputSchema: obj({ from: str(''), to: str(''), tag: str(''), limit: num('默认 50') }) },
  ...
  {
    name: 'auto_recall',
    description: '自动召回（给"每条消息自动注入"用，钩子走 REST /api/recall 调它）：两层级联——先纹理关键词命中（authority），一条都没有再翻年轮（last_resort）。服务器设了 MUWEN_AUTORECALL_AGENT=1 就先跑一次模型把消息扩写成 2-4 个检索角度再搜（"暗号"能找到"项圈还在吗"），每个角度各搜一遍按 id 合并。琐碎消息（"嗯""好的"）返回空。辞平时用 recall 就行，这个主要是后台自动跑的。',
    inputSchema: obj({ query: str('拿来匹配记忆的文本，一般是棋子的消息'), max_return: num('默认 3'), use_agent: { type: 'boolean', description: '是否先跑一次模型扩写检索角度，默认看服务器 MUWEN_AUTORECALL_AGENT' }, queries: strArr('直接给检索角度，跳过模型扩写（调试/预扩写用）') }, ['query'])
  },
...
export async function callMuwenTool(name, args = {}) {
  switch (name) {
    ...
    case 'auto_recall': need(args, 'query'); return recall.autoRecall(args.query, { maxReturn: args.max_return || 3, useAgent: args.use_agent === undefined ? null : args.use_agent, queries: args.queries || null });
