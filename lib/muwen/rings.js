...
export function searchRingOccurrences({ query, limit = 60, perRing = 5, radius = 90, from, to, window_name } = {}) {
  ...
  for (const r of pool) {
    ...
    while (idx !== -1) {
      total++;
      if (n < perRing && hits.length < limit) {
        const start = Math.max(0, idx - radius);
        const end = Math.min(r.content.length, idx + q.length + radius);
        hits.push({
          ...
          before: (start > 0 ? '…' : '') + r.content.slice(start, idx),
          match: r.content.slice(idx, idx + q.length),
          after: r.content.slice(idx + q.length, end) + (end < r.content.length ? '…' : '')
        });
        n++;
      }
  ...
  return { query: q, total, shown: hits.length, hits };
}
