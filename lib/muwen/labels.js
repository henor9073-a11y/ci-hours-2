...
export const LABELS = [
  ...
  // ---- 人际 ----
  { key: 'social', name: '人际关系', group: '人际', desc: '一个人的卡片：name 必填，gender/relation/owner(nor|cy|shared)/intro/status/text；同名再写一次就是更新（状态会变）', tool: 'update_social', need: ['name'], aliases: ['人际', '人', '朋友'], build: a => ({ name: a.name || a.title, gender: a.gender, relation: a.relation, owner: a.owner, intro: a.intro, status: a.status, text: a.text, aliases: a.aliases, rename: a.rename }) },
  { key: 'gossip', name: '八卦', group: '人际', desc: '关于某个人的八卦：about 对应人际关系里的 name', tool: 'add_gossip', need: ['text'], dateDefault: true, aliases: ['瓜'], build: a => ({ about: a.about, text: a.text, date: a.date }) },
