...
// 显示名统一从标签总表来，别在这里另写一份（以前写的"学到的"跟前端"学习"对不上）
export const CATEGORY_LABELS = Object.fromEntries(CATEGORIES.map(c => [c, labelName(c)]));
...
export function touch(ids, kind = 'read') {
  ...
  const d = load();
  const touched = [];
  // 木纹网页只读：照常返回，但热度、访问次数一概不动
  if (isReadonly()) return d.grains.filter(g => list.includes(g.id)).map(decorate);
...
// ---- 搜索 ----
// sort：heat（默认，热度高的在前）/ time_desc（新的在前）/ time_asc（旧的在前）。
// 时间用 date（事情发生那天）；老数据一半没有 date，退回 created_at 那天——迁移时保留了原来的创建时间，大体对得上。
export const SORTS = ['heat', 'time_desc', 'time_asc'];
export function timeKeyOf(g) { return (g.date || String(g.created_at || '').slice(0, 10)) + ' ' + (g.created_at || ''); }
export function searchGrains({ query, category, family, from, to, status, limit = 20, touchHits = true, sort = 'heat' } = {}) {
  const d = load();
  let pool = d.grains;
  if (category) pool = pool.filter(g => g.category === category);
  if (family) pool = pool.filter(g => g.families.includes(family));
  if (status) pool = pool.filter(g => g.status === status);
  // 归档的只有明确传 status='archived' 才搜得到，跟工具说明一致。
  // 以前是"带了 query 就一起搜"，结果自动召回（天天带 query）一直在把辞已经
  // 决定不要的东西端上来——实测有一条 archived、heat 60 的纹理天天被召回。
  // "归档 = 明确不要了，不管 heat 多高都不主动返回"，这里是那句话的唯一执行点。
  else pool = pool.filter(g => g.status !== 'archived');
  ...
  // 按时间排是"浏览"不是"找最像的"：带了 query 也只在命中的里面按时间排
  if (sort === 'time_desc') scored.sort((a, b) => timeKeyOf(b.g).localeCompare(timeKeyOf(a.g)));
  else if (sort === 'time_asc') scored.sort((a, b) => timeKeyOf(a.g).localeCompare(timeKeyOf(b.g)));
