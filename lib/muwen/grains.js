...
// ---- 搜索 ----
export function searchGrains({ query, category, family, from, to, status, limit = 20, touchHits = true, includeArchived = true } = {}) {
  const d = load();
  let pool = d.grains;
  if (category) pool = pool.filter(g => g.category === category);
  if (family) pool = pool.filter(g => g.families.includes(family));
  if (status) pool = pool.filter(g => g.status === status);
  // 归档的只有明确搜的时候才出现。includeArchived=false 是给自动召回用的：
  // "归档 = 明确不要了，不管 heat 多高都不主动返回"——自动召回正是"主动返回"，
  // 不该把辞已经决定不要的东西又端上来（实测有一条 archived、heat 60 的纹理天天被召回）。
  else pool = pool.filter(g => g.status !== 'archived' || (!!query && includeArchived));
