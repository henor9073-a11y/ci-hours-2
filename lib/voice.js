...
// 2026-09-11 从 eleven_multilingual_v2 换到 eleven_v3。
// 留成环境变量是因为 v3 对 voice_settings 的接受范围跟 v2 不一样（见下面的退一步重试），
...
// 语速/音色参数——之前语速太快了，调慢一点。
const VOICE_SETTINGS = { stability: 0.6, similarity_boost: 0.5, style: 0, speed: 0.73 };

async function callEleven(text, settings) {
  return fetch(`https://api.elevenlabs.io/v1/text-to-speech/${VOICE_ID}`, {
    method: 'POST',
    headers: { 'xi-api-key': API_KEY, 'Content-Type': 'application/json' },
    body: JSON.stringify({ text, model_id: MODEL_ID, ...(settings ? { voice_settings: settings } : {}) })
  });
}

export async function synthesizeSpeech(text) {
  if (!API_KEY || !VOICE_ID) throw new Error('没配置 ELEVENLABS_API_KEY / ELEVENLABS_VOICE_ID');
  let res = await callEleven(text, VOICE_SETTINGS);
  ...
  if (!res.ok && res.status >= 400 && res.status < 500) {
    ...
    if (/voice_settings|stability|speed|similarity/i.test(first)) {
      ...
      res = await callEleven(text, null);
    } else {
