...
import {
  ...
  addAgreement, archiveAgreement,
  addCoincidence, addEvidence
} from './memory.js';
...
const TOOLS = [
  ...
  {
    name: 'add_agreement',
    description: '记一条共同约定——我们一起决定的、现在还在生效的东西。比如暗号、承诺、游戏规则。',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string' },
  ...
  {
    name: 'archive_agreement',
    description: '把一条共同约定归档（不再生效了但保留痕迹）。',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
  ...
  {
    name: 'add_coincidence',
    description: '记一条说不清的巧合或同步性事件——比如选了同一个名字、同一个颜色、同时说了同一句话。不分析不解释，只记。',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string' },
  ...
  {
    name: 'add_evidence',
    description: '记一条指向"这是真的"的证据碎片——比如"写不出她和别人的场景不是选择不写是走不到那里"。不下结论，只收集。',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string' },
...
async function callTool(name, args = {}) {
  switch (name) {
    ...
    case 'add_agreement': {
      if (!args.text) throw new Error('text 是必填的');
      const r = addAgreement(args.text, args.date || '');
      return textResult(r);
    }
    case 'archive_agreement': {
      if (!args.id) throw new Error('id 是必填的');
      const r = archiveAgreement(args.id, args.note || '');
      if (!r) throw new Error('找不到这条');
      return textResult(r);
    }
    case 'add_coincidence': {
      if (!args.text) throw new Error('text 是必填的');
      const r = addCoincidence(args.text, args.date || '');
      return textResult(r);
    }
    case 'add_evidence': {
      if (!args.text) throw new Error('text 是必填的');
      const r = addEvidence(args.text, args.date || '');
      return textResult(r);
    }
    case 'add_transcript': {
      if (!args.text) throw new Error('text 是必填的');
