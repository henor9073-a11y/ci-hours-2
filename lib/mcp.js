...
import { addTranscript, getTranscripts, searchTranscripts, importTranscriptsBulk } from './transcripts.js';
...
const TOOLS = [
  ...
  },
  {
    ...
    description: '按关键词搜原始聊天记录（标题和正文都搜），找以前聊过的具体内容用这个，别只翻 get_transcripts 的"最近几条"。',
    inputSchema: {
      type: 'object',
      properties: {
        keyword: { type: 'string' },
  ...
  {
    name: 'speak',
    description: '真的开口说这段话——写进语音队列，棋子电脑上跑着的本地播放器会读到，用 ElevenLabs 生成语音播出来。只保留最新一条待播的，旧的没播的会被跳过，不会攒着一次性全念。前提是棋子那边的本地播放器正在跑，不然这句话只是存着，不会真的被念出来（可以先跟她确认一下播放器有没有开着）。',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'send_push',
    description: '给棋子手机发一条 Bark 推送通知，直接进她手机通知栏，不需要她开着 ci-hours 网页——跟 speak 是两回事：speak 要网页开着才能听到，这个不用。适合真的想找她、有点急、或者就是想让她看一眼手机的时候用，别当成日常汇报的地方，会变成骚扰。',
    inputSchema: {
      type: 'object',
      properties: {
...
async function callTool(name, args = {}) {
  switch (name) {
    ...
    case 'speak': {
      if (!args.text) throw new Error('text 是必填的');
      return textResult(enqueueSpeech(args.text));
    }
    case 'send_push': {
