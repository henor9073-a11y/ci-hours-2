...
import { getQuestions, askQuestion, replyToQuestion, resolveQuestion } from './memory.js';
import { TOOLS as MUWEN_TOOLS, callMuwenTool } from './muwen/tools.js';
...
import { LABELS, findLabel, buildArgs, publicLabels } from './muwen/labels.js';
import { runReadonly, READONLY_TOOL } from './muwen/readonly.js';
...
// 木纹（muwen）：ci-hours 改造之后的名字。记忆相关的工具都在 muwen/tools.js 里，排在最前面（按醒来流程排序）。
const SERVER_INFO = { name: 'muwen', version: '2.0.0' };
const PROTOCOL_VERSION = '2024-11-05';

const REMEMBER_DESC = '写记忆/记录的统一入口。**每次写的时候标上 label**，后端按标签自动放到对应的地方，网页也按同一张标签表分类，两边永远一致。' +
  'label 写英文 key 或中文名都行；认不出来会报错并列出全部标签，不会乱放。\n\n可用标签（key=中文名：说明 · 至少要给的字段）：\n' +
  LABELS.map(l => `· ${l.key}=${l.name}：${l.desc}${l.need && l.need.length ? ' · ' + l.need.join('/') : ''}`).join('\n') +
  '\n\n常用字段直接传（text/date/title）；某个标签特有的字段（sleep 的 sleepTime/wakeTime、schedule 的 time、emotion 的 emotion、profile 的 field/reason、song 的 artist/lyrics……）放进 fields。日期没给的，需要日期的标签默认今天。';
...
const TOOLS = [
  ...
  },
  {
    ...
    description: '写一篇日记，自己决定醒来的时候要不要写、写什么。visibility 自己选：public 棋子能看到，private 只有你自己能看到（棋子那边的网页/接口不会吐出私密日记）。不是每次醒来必须写，想写的时候写。category 默认 diary（辞的日记）；写观察棋子的传 wife_observation（妻子观察日记，跟日记分开存），或者直接用 add_wife_observation。',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string' },
        visibility: { type: 'string', enum: ['public', 'private'], description: '默认 public' },
        category: { type: 'string', enum: ['diary', 'wife_observation'], description: '默认 diary（辞的日记）；wife_observation = 妻子观察日记' },
        date: { type: 'string', description: 'YYYY-MM-DD，写的是哪天，可选' }
      },
      required: ['text'],
      additionalProperties: false
    }
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { limit: { type: 'number', description: '最多返回几条，默认 50' }, category: { type: 'string', enum: ['diary', 'wife_observation'], description: '默认 diary' } },
      additionalProperties: false
    }
  },
  {
    name: 'add_wife_observation',
    description: '写一篇妻子观察日记：你观察棋子、写她的。跟自己的日记分开存，网页"日记"里切到"妻子观察日记"能看到公开的。没有自动总结，都是你自己写。',
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string' },
        date: { type: 'string', description: 'YYYY-MM-DD，观察的是哪天，可选' },
        visibility: { type: 'string', enum: ['public', 'private'], description: '默认 public' }
      },
      required: ['text'],
      additionalProperties: false
  ...
  {
    name: 'get_wife_observations',
    description: '读自己写过的妻子观察日记（公开和私密都有）。',
    inputSchema: { type: 'object', properties: { limit: { type: 'number', description: '默认 50' } }, additionalProperties: false }
  },
  {
    name: 'list_labels',
    description: '看标签总表：木纹里所有能写进去的记录类型——每个标签是什么、放到哪、至少要给哪些字段。用 remember 之前不确定标签就先看这个。',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false }
  },
  {
    name: 'remember',
    ...
    inputSchema: {
      type: 'object',
      properties: {
        label: { type: 'string', description: '标签：英文 key 或中文名（见上面的表）' },
        text: { type: 'string', description: '正文' },
        date: { type: 'string', description: 'YYYY-MM-DD，可选；需要日期的标签默认今天' },
        title: { type: 'string', description: '标题，first/song/countdown 这类要' },
        name: { type: 'string', description: 'social：这个人的称呼' },
        gender: { type: 'string', description: 'social：性别' },
        relation: { type: 'string', description: 'social：跟谁的关系' },
        owner: { type: 'string', description: 'social：nor/cy/shared' },
        intro: { type: 'string', description: 'social：一两句简介' },
        status: { type: 'string', description: 'social：当前关系状态' },
        about: { type: 'string', description: 'gossip：关于谁（对应 social 的 name）' },
        fields: { type: 'object', description: '这个标签特有的其他字段，原样交给对应工具' }
      },
      required: ['label'],
      additionalProperties: false
    }
...
async function callTool(name, args = {}) {
  const mwResult = await callMuwenTool(name, args);
  if (mwResult !== undefined) return textResult(mwResult);
  switch (name) {
    // get_state 是最常被拿来"看看现在什么情况"的工具之一，欢迎语也放进来，不止 get_identity 里才有。
    ...
    case 'add_diary_entry': {
      ...
      return textResult(addDiaryEntry(args.text, args.visibility || 'public', args.category || 'diary', args.date || ''));
    }
    case 'get_diary': return textResult(getDiaryAll(args.limit || 50, args.category || 'diary'));
    case 'add_wife_observation': {
      if (!args.text) throw new Error('text 是必填的');
      return textResult(addDiaryEntry(args.text, args.visibility || 'public', 'wife_observation', args.date || ''));
    }
    case 'get_wife_observations': return textResult(getDiaryAll(args.limit || 50, 'wife_observation'));
    case 'list_labels': return textResult(publicLabels());
    case 'remember': {
      const label = findLabel(args.label);
      if (!label) throw new Error(`认不出标签「${args.label || ''}」。可用的：${LABELS.map(l => `${l.key}(${l.name})`).join('、')}`);
      const res = await callTool(label.tool, buildArgs(label, args));
      if (res.isError) throw new Error(res.content[0].text);
      let inner = res.content[0].text; try { inner = JSON.parse(inner); } catch {}
      return textResult({ label: label.key, label_name: label.name, stored_via: label.tool, result: inner });
    }
    ...
    try {
      ...
      switch (msg.method) {
        ...
        case 'tools/call': {
          ...
          // 木纹网页发来的：只能读，而且读也不改记忆（不升温、不记访问、不写召回日志）
          const readonly = !!req.headers['x-muwen-readonly'];
          try {
            if (readonly && !READONLY_TOOL(name)) throw new Error(`木纹网页是只读的，不能调 ${name}`);
            result = readonly ? await runReadonly(() => callTool(name, args || {})) : await callTool(name, args || {});
          } catch (e) {
