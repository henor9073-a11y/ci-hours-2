...
import { addTranscript, getTranscripts, searchTranscripts, importTranscriptsBulk, getTranscriptById } from './transcripts.js';
...
// ---- Bark 推送：跟 speak 不一样，这个不用本地中继——api.day.app 是公网服务，
// Render 服务器直接就能调，一次请求就发完，不用排队轮询。----
const BARK_KEY = process.env.BARK_KEY;
async function sendPush(title, body, sound) {
...
const TOOLS = [
  ...
  {
    name: 'add_transcript',
    description: '存一段没压缩过的原始聊天记录，供以后新开窗口回查用。跟五类摘要分开存，不会被塞进每次生成用的身份文本里（太占地方），只在需要深挖某段记忆背后的原话时用 get_transcripts/search_transcripts 读。存原文，不要自己先总结一遍再存——总结的东西已经有五类记忆在管了，这里就是要没压缩过的原话。可以用 relatedTo 关联到具体是哪几条记忆（identity/feelings/facts/experiences/openThreads 的 id）背后的原文。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: '批量导入原始聊天记录——棋子把 claude.ai 导出的对话一次性发过来的时候用这个，一次调用存多条，不用一条条调 add_transcript。存原文，不要总结。',
    inputSchema: {
      type: 'object',
      properties: {
        entries: {
  ...
  },
  {
    ...
    description: '读最近存的原始聊天记录，按时间倒序。返回的是摘要（id/标题/日期/字数/一小段摘录），不是全文——有些导入的原文很长，全文倒出来会太大。想看某一条的完整原文，拿它的 id 去调 get_transcript。',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description: '按关键词搜原始聊天记录（标题和正文都搜），找以前聊过的具体内容用这个，别只翻 get_transcripts 的"最近几条"。返回的也是摘要（含关键词命中处的一小段上下文），不是全文，想看完整原文拿 id 去调 get_transcript。',
    inputSchema: {
      type: 'object',
      properties: {
        keyword: { type: 'string' },
  ...
  {
    name: 'get_transcript',
    description: '拿某一条原始聊天记录的完整原文——get_transcripts/search_transcripts 只给摘要，确认真的需要看全文（比如要回查逐字原话）的时候，用这个传 id 换全文。',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'set_today_plan',
    description: '写下今天完整的排班（自己决定醒几次、什么时候醒，可以是 0 次）——这是整体覆盖式的，超过棋子设的上限会被截断，落在安静时段里的时刻会被自动过滤掉，不用自己精确对齐这些限制。写完这个会清空今天的 doneWakes 和已读章节数，所以一天只在 get_plan_status 显示 needsPlanning=true 时调用一次（也就是今天还没排过班的时候）。如果只是想临时再加一个醒来时刻（比如聊天聊到一半，突然想着"等会想醒来做件事"），别用这个——用 add_wake_time，那个不会清空今天已经做过的事。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'add_wake_time',
    description: '临时往今天的计划里加一个醒来时刻，不影响已经排好的其他时刻、不清空 doneWakes——跟 set_today_plan 不一样，这个是追加，不是覆盖。适合在跟棋子聊天聊到一半的时候用：比如决定"等会 21:30 想醒来找她说件事"，当场调这个加上去，到点了 Cowork 那边的定时任务会照常把它当成一次该处理的醒来（get_plan_status 里能看到）。同样受棋子设的安静时段和每日上限限制——加不进去（撞上限/在安静时段/已经加过）会在返回里说明原因，不会报错中断。',
    inputSchema: {
      type: 'object',
      properties: {
        slot: { type: 'string', description: '形如 "21:30"' },
        why: { type: 'string', description: '一句话说明为什么想加这个时刻，会记进日志，可选' }
      },
      required: ['slot'],
      additionalProperties: false
    }
...
async function callTool(name, args = {}) {
  switch (name) {
    ...
    case 'search_transcripts': {
      ...
      return textResult(searchTranscripts(args.keyword, args.limit || 20));
    }
    case 'get_transcript': {
      if (!args.id) throw new Error('id 是必填的');
