# 木纹自动召回注入钩子（Windows / Claude Code UserPromptSubmit）。
...
try {
  $body = @{ query = $p } | ConvertTo-Json -Compress
  $r = Invoke-RestMethod -Method Post -Uri "$url/api/recall?token=$token" -ContentType 'application/json; charset=utf-8' -Body ([Text.Encoding]::UTF8.GetBytes($body)) -TimeoutSec $timeout
  if ($r) { Write-Output $r }   # 服务器已经拼好 [muwen:recall] … 文本；没匹配到就是空，什么都不注入
